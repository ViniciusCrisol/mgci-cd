...
function handleError(error: unknown, errorPrefix: string): never {
	...
	if (errorMessage.includes("403 Forbidden")) throw new ForbiddenError(`${errorPrefix}: ${errorMessage}`);
	if (errorMessage.includes("404 Not Found")) throw new NotFoundError(`${errorPrefix}: ${errorMessage}`);
	if (errorMessage.includes("409 Conflict")) throw new ValidationError(`${errorPrefix}: ${errorMessage}`);
